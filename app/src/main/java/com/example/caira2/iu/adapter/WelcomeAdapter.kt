...
import com.example.caira2.repository.WelcomeList.welcomeFrames

class WelcomeAdapter(fragment: FragmentActivity) : FragmentStateAdapter(fragment) {

    override fun getItemCount(): Int = welcomeFrames.size

    override fun createFragment(position: Int): Fragment {
        // Return a NEW fragment instance in createFragment(int)
        val fragment = Welcome_item_Fragment()
        fragment.arguments = Bundle().apply {
            putSerializable(ARG_OBJECT, welcomeFrames[position])
        }
        return fragment
    }
}