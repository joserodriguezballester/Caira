...
////Mapeado
            ...
            val responseError = RetrofitInstance.getRestApiServices().add_user2(user)
            ...
            if (responseError.code()==400){
                val data = Gson().fromJson(jsonObject.toString(), Detail400::class.java)
                Log.i("*****2", "response.json=====--${data.detail}")
            }else{
                val data = Gson().fromJson(jsonObject.toString(), Details::class.java)
                Log.i("*****2", "response.json=====--${data.detail}")
            }
